...
var loadState = {
	...
	preload: function () {
		...
		var loadingLabel = game.add.text(game.world.centerX, game.world.centerY, 'loading...', { font: '30px Courier', fill: '#FFFFFF' } );
		...
		game.load.physics('physics', 'res/physics/physics.json');
...
var menuState = {
	...
	create: function() {
		var titleText = game.add.text(200, 150, 'Super Duck Flap', { font: '48px Arial', fill: '#000' });
		titleText.anchor.setTo(0.5, 0.5);
		titleText.angle = (2 + Math.random() * 5) * (Math.random() > 0.5 ? 1 : -1);
		var titleTween = game.add.tween(titleText);
		titleTween.to( { angle: -titleText.angle }, 5000 + Math.random() * 5000, Phaser.Easing.Linear.None, true, 0, 1000, true);
		
		var playButton = game.add.button(200, 350, 'playButton', this.onPlayPressed, this);
...
var preMainState = {
	...
	create: function() {
		var bird = game.add.sprite(100, game.world.centerY, 'duck');
		bird.anchor.setTo(-0.2, 0.5);
		
		var startText = game.add.text(150, game.world.centerY - 30, 'Flap to Start!', { font: '30px Arial', fill: '#000000' });
...
var mainState = {
	...
	create: function() {
		...
		this.bird.body.loadPolygon('physics', 'bird');
		...
		score = 0;
		this.labelScore = game.add.text(game.world.centerX, 20, '0', { font: '30px Arial', stroke: '#000000', fill: '#FFFFFF' });
		...
		this.bird.body.collides([
			this.pipeCollisionGroup
		]);
	},
	...
	update: function() {
		...
		this.pipes.forEach(function (child) {
			child.body.clearShapes();
			child.body.loadPolygon('physics', 'pipe');
		}, this);
	},
	...
	addOnePipe: function(x, y, pos) {
		...
		pipe.body.setCollisionGroup(this.pipeCollisionGroup);
		pipe.body.collides([
